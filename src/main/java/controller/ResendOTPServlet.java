...
public class ResendOTPServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // goi cac doi tuong email , user , lay session account
        Email emailRe = new Email();
        User us = new User();
        us = (User) session.getAttribute("Account");

        // set new otp
        int otpNew = emailRe.sendEmailOTP(us.getEmail());
        session.removeAttribute("OTP");
        session.setAttribute("OTP", otpNew);

        // send success resend otp
        session.setAttribute("newOTP", "Success Resend OTP");
